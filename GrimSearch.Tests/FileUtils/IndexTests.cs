...
namespace GrimSearch.Tests.FileUtils
{
    ...
    public class IndexTests
    {
        ...
        [TestMethod]
        public async Task FindsLocalizedAndEnglishItemNames()
        {
            ...
            {
                ["tagWeaponMelee2hD010"] = "ウルテスの嵐探し",
                ["tagStyleUniqueTier3"] = "神話級"
            };
            StringsCache.Instance.SetStringsForTesting(localized, english, "JA");
            var index = new Index();
            ...
            var localizedResults = await index.FindAsync("嵐探し", filter);
            var englishResults = await index.FindAsync("stormseeker", filter);

            Assert.IsTrue(localizedResults.Results.Any(x => x.ItemName == "神話級 ウルテスの嵐探し"));
            Assert.IsTrue(englishResults.Results.Any(x => x.ItemName == "神話級 ウルテスの嵐探し"));
        }
